...
async function persistCurrentSessionCookies(messageTransport: MessageTransport) {
    if (!browser || !cookiesDirPath || !cookiesFilePath) return
    ...
    const cookiesObject = await studioPage.cookies(
...
export const upload = async (
    ...
    try {
        ...
        if (useCookieStore) await persistCurrentSessionCookies(messageTransport)
        await browser.close()
...
async function uploadVideo(videoJSON: Video, messageTransport: MessageTransport) {
    ...
    if (videoJSON.publishType) {
        ...
        if (publishRadio) {
            ...
            const publishOptions = await page.$x(
                `//*[normalize-space(text())='${publishLabel}']/ancestor-or-self::*[@role='radio' or self::tp-yt-paper-radio-button][1]`
            )
            if (publishOptions.length === 0) {
                throw new Error(`YouTube Studio ${publishLabel} visibility option not found`)
            }
            await page.evaluate((option) => (option as HTMLElement).click(), publishOptions[0])
        }
    ...
    do {
        ...
        (uploadedLink === videoBaseLink || uploadedLink === shortVideoBaseLink) &&
...
async function waitForStudioEditorPage(): Promise<Page> {
    ...
    const refreshedBrowser = await puppeteer.connect({ browserWSEndpoint: endpoint })
    browser.disconnect()
