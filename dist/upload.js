...
async function uploadVideo(videoJSON, messageTransport) {
    ...
    if (!fileInput)
        throw new Error('YouTube Studio upload input not found');
    ...
    if (!advancedEditorReady) {
        const showMoreButtons = await page.$x("//*[normalize-space(text())='Show more']/ancestor-or-self::*[self::ytcp-button or self::button or @role='button'][1]");
...
async function loadAccount(credentials, messageTransport, useCookieStore = true) {
    ...
    if (hasSavedCookies) {
        ...
        try {
            ...
            throw new Error(`Saved YouTube session did not reach Studio within ${timeout / 1000}s (landed on ${hostname}). Authenticate again only if the landing domain is accounts.google.com.`);
        }
    }
    try {
...
async function waitForStudioEditorPage() {
    ...
    while (Date.now() < deadline) {
        for (const candidate of await browser.pages()) {
            if (!candidate.url().includes('studio.youtube.com'))
                continue;
            const state = await candidate.evaluate(() => {
                var _a;
                return ({
                    dailyLimit: ((_a = document.body) === null || _a === void 0 ? void 0 : _a.innerText.includes('Daily upload limit reached')) || false,
                    textboxes: document.querySelectorAll('[id="textbox"]').length,
                    hasMetadataEditor: Boolean(document.querySelector('ytcp-video-metadata-editor-basics') ||
                        document.querySelector('ytcp-uploads-dialog'))
                });
            }).catch(() => null);
            if (state === null || state === void 0 ? void 0 : state.dailyLimit)
